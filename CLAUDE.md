...
- **`InductiveSmartContract`** — Extends `StatefulSmartContract` with automatic genesis tracking and a ZKP proof placeholder. Two internal properties are auto-injected: `_genesisOutpoint` (36 bytes, immutable token identity) and `_proof` (256 bytes, ZKP stub). Genesis detection uses a 36-byte zero sentinel — on the first spend, `_genesisOutpoint` is set to the current outpoint; on subsequent spends it is preserved unchanged. Uses the same implicit unlock params as `StatefulSmartContract` (`_changePKH`, `_changeAmount`, `_newAmount`, `txPreimage`). Same output ordering as `StatefulSmartContract` (covenant first, change last). See `docs/inductive-contracts.md` for full details.
