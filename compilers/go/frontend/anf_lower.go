...
type lowerCtx struct {
	...
	// construction. nil = default "lowS". Propagated into sub-contexts like sighashFlag.
...
func (ctx *lowerCtx) subContext() *lowerCtx {
	sub := &lowerCtx{
		...
		bindingVariant:   ctx.bindingVariant,
...
func (ctx *lowerCtx) lowerExprToRef(expr Expression) string {
	...
	case BinaryExpr:
		...
		// stack ops, so BOTH operands were pushed and therefore both
		// evaluated. spec/semantics.md §3.7 licensed that with "This is safe
		// in Rúnar because all expressions are pure (no side effects beyond
		// assert)". Purity is not TOTALITY: the same document's §10 and §11.3
		// list division by zero as a runtime failure, and OP_SPLIT /
