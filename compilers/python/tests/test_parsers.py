...
class TestRubyIntegration:
    ...
    def test_all_example_contracts_parse_without_errors(self):
        ...
        for path in sorted(rb_files):
            ...
            errors = [e for e in result.errors if "error" in e.message.lower()] if result.errors else []
            if result.contract is None:
                failures.append(f"{file_name}: no contract returned, errors={result.error_strings()}")
            elif result.errors:
                # Some warnings are acceptable; only flag hard errors
                hard = [e for e in result.errors if "error" in e.message.lower()]
                if hard:
