...
is **9425 bytes** (18850 hex chars). The byte count grew to 9425 with the
GAP-302 stateful-covenant hardening: each public method now carries a
fixed 760-byte, sighash-pinned checkPreimage blob (OP_CHECKSIG lives
inside the blob). (Historically the size was 5087 bytes, up from 4951
...
class TestTicTacToeCrossCompilerBytes:
    def test_canonical_ts_v1_locks_to_9425_bytes(self):
    ...
    def test_canonical_ts_v2_locks_to_9425_bytes(self):
    ...
    def test_python_dsl_source_locks_to_9425_bytes(self):
