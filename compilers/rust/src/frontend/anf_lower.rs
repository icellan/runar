...
fn lower_methods(contract: &ContractNode) -> Vec<ANFMethod> {
    ...
    for method in &contract.methods {
            ...
            && method.visibility == Visibility::Public
        {
            ...
            let has_mutable_state = contract.properties.iter().any(|p| !p.readonly);
            if has_mutable_state {
                let ds_preimage = method_ctx.emit(ANFValue::LoadParam {
                    name: "txPreimage".to_string(),
                });
                method_ctx.emit(ANFValue::DeserializeState {
                    preimage: ds_preimage,
                });
            }
            ...
            genesis_ctx.emit(ANFValue::Call {
                func: "extractOutpoint".to_string(),
                args: vec![gp_ref],
            });
            method_ctx.sync_counter(&genesis_ctx);

            // Non-genesis branch: verify snark proof, then produce existing _genesisOutpoint
            let mut non_genesis_ctx = method_ctx.sub_context();
            let proof_ref = non_genesis_ctx.emit(ANFValue::LoadProp {
                name: "_proof".to_string(),
            });
            let ng_genesis_ref = non_genesis_ctx.emit(ANFValue::LoadProp {
                name: "_genesisOutpoint".to_string(),
            });
            let snark_result = non_genesis_ctx.emit(ANFValue::SnarkVerify {
                proof: proof_ref,
                public_inputs: vec![ng_genesis_ref.clone()],
            });
            non_genesis_ctx.emit(ANFValue::Assert {
                value: snark_result,
            });
            non_genesis_ctx.emit(ANFValue::LoadProp {
                name: "_genesisOutpoint".to_string(),
            });
            method_ctx.sync_counter(&non_genesis_ctx);

            // Emit the if/else — result is the genesis outpoint to use
            let genesis_outpoint_result = method_ctx.emit(ANFValue::If {
                cond: is_genesis,
            ...
            // Output order: same as StatefulSmartContract (covenant first, change last).
            let add_output_refs = method_ctx.add_output_refs.clone();
            if !add_output_refs.is_empty() || needs_change_output {
                ...
                if !add_output_refs.is_empty() {
                    ...
                    // Append change after covenant outputs
                    ...
                    let contract_output_ref = method_ctx.emit(ANFValue::Call {
                        ...
                        args: vec![preimage_ref2, state_script_ref, new_amount_ref],
                    });
                    // Covenant first, then change
                    let all_outputs = method_ctx.emit(ANFValue::Call {
                        func: "cat".to_string(),
            ...
            // Append implicit params: same as StatefulSmartContract
    ...
    if let Expression::PropertyAccess { property } = callee {
        if property == "addOutput" {
            ...
            if ctx.contract.parent_class == "InductiveSmartContract" {
                ...
                let proof_ref = ctx.emit(ANFValue::LoadProp { name: "_proof".to_string() });
                state_values.push(genesis_ref);
                state_values.push(proof_ref);
            }
