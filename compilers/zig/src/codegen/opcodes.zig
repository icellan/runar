...
/// Replaces ArrayList.writer(allocator) which was removed in Zig 0.16.
...
test "push data encoding — empty" {
    ...
    const w_pd_1 = ArrayListWriter{ .list = &buf, .allocator = std.testing.allocator };
    try encodePushData(w_pd_1, &.{});
...
test "push data encoding — short (2 bytes)" {
    ...
    const w_pd_2 = ArrayListWriter{ .list = &buf, .allocator = std.testing.allocator };
    try encodePushData(w_pd_2, &.{ 0xab, 0xcd });
...
test "push data encoding — 75 bytes (max direct)" {
    ...
    @memset(&data, 0xff);
    const w_pd_3 = ArrayListWriter{ .list = &buf, .allocator = std.testing.allocator };
    try encodePushData(w_pd_3, &data);
...
test "push data encoding — 76 bytes (PUSHDATA1)" {
    ...
    @memset(&data, 0xaa);
    const w_pd_4 = ArrayListWriter{ .list = &buf, .allocator = std.testing.allocator };
    try encodePushData(w_pd_4, &data);
...
test "push data encoding — 256 bytes (PUSHDATA2)" {
    ...
    @memset(&data, 0xbb);
    const w_pd_5 = ArrayListWriter{ .list = &buf, .allocator = std.testing.allocator };
    try encodePushData(w_pd_5, &data);
...
test "script number encoding — zero" {
    ...
    const w_sn_1 = ArrayListWriter{ .list = &buf, .allocator = std.testing.allocator };
    try encodeScriptNumber(w_sn_1, 0);
    try std.testing.expectEqualSlices(u8, &.{0x00}, buf.items);
}
...
test "script number encoding — small positives (1-16)" {
    ...
    const w_sn_2 = ArrayListWriter{ .list = &buf, .allocator = std.testing.allocator };
    try encodeScriptNumber(w_sn_2, 1);
    try std.testing.expectEqualSlices(u8, &.{0x51}, buf.items);

    buf.clearRetainingCapacity();
    const w_sn_3 = ArrayListWriter{ .list = &buf, .allocator = std.testing.allocator };
    try encodeScriptNumber(w_sn_3, 16);
    try std.testing.expectEqualSlices(u8, &.{0x60}, buf.items);

    buf.clearRetainingCapacity();
    const w_sn_4 = ArrayListWriter{ .list = &buf, .allocator = std.testing.allocator };
    try encodeScriptNumber(w_sn_4, 8);
...
test "script number encoding — negative one" {
    ...
    const w_sn_5 = ArrayListWriter{ .list = &buf, .allocator = std.testing.allocator };
    try encodeScriptNumber(w_sn_5, -1);
...
test "script number encoding — 17 (beyond OP_16)" {
    ...
    const w_sn_6 = ArrayListWriter{ .list = &buf, .allocator = std.testing.allocator };
    try encodeScriptNumber(w_sn_6, 17);
...
test "script number encoding — 128 (needs sign byte)" {
    ...
    const w_sn_7 = ArrayListWriter{ .list = &buf, .allocator = std.testing.allocator };
    try encodeScriptNumber(w_sn_7, 128);
...
test "script number encoding — -128 (negative, MSB conflict)" {
    ...
    const w_sn_8 = ArrayListWriter{ .list = &buf, .allocator = std.testing.allocator };
    try encodeScriptNumber(w_sn_8, -128);
...
test "script number encoding — 255" {
    ...
    const w_sn_9 = ArrayListWriter{ .list = &buf, .allocator = std.testing.allocator };
    try encodeScriptNumber(w_sn_9, 255);
...
test "script number encoding — -5" {
    ...
    const w_sn_10 = ArrayListWriter{ .list = &buf, .allocator = std.testing.allocator };
    try encodeScriptNumber(w_sn_10, -5);
...
test "script number encoding — large positive 1000" {
    ...
    const w_sn_11 = ArrayListWriter{ .list = &buf, .allocator = std.testing.allocator };
    try encodeScriptNumber(w_sn_11, 1000);
...
test "script number encoding — i64 MIN does not panic" {
    ...
    const w_sn_12 = ArrayListWriter{ .list = &buf, .allocator = std.testing.allocator };
    try encodeScriptNumber(w_sn_12, std.math.minInt(i64));
