...
test "C27: checkPreimage reachable only through a private helper still emits _codePart" {
    ...
    // Cross-tier lock: TS, Go, Rust, Python, Ruby and Java all emit 619 bytes
    // for this program (872 with the legacy 760-byte binding blob; the Any-S
    // blob is 332 bytes smaller; R-010's hoisted OP_CODESEPARATOR prologue plus
    // the `_codePart` authentication sequence add 76 more; R-095's
    // `verify_code_part_len` pin adds 9 to the one authenticating method, this
    // contract's state being a variable-length ByteString; C=1 shrinks the
    // binding blob by another 6). Zig under-emitted while the private-method
    // map was null. All seven tiers were re-measured at 619 on the same ANF
    // input before this number was moved.
    const hex = try scriptHex(artifact);
    try std.testing.expectEqual(@as(usize, 619 * 2), hex.len);
}
