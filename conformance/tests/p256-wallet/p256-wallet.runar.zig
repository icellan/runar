...
pub const P256Wallet = struct {
    ...
    pub fn init(ecdsaPubKeyHash: runar.Addr, p256PubKeyHash: runar.ByteString) P256Wallet {
        return .{ .ecdsaPubKeyHash = ecdsaPubKeyHash, .p256PubKeyHash = p256PubKeyHash };
    }

    pub fn spend(self: *const P256Wallet, p256Sig: runar.ByteString, p256PubKey: runar.ByteString, sig: runar.Sig, pubKey: runar.PubKey) void {
        runar.assert(runar.hash160(pubKey) == self.ecdsaPubKeyHash);
        runar.assert(runar.checkSig(sig, pubKey));
        runar.assert(runar.hash160(p256PubKey) == self.p256PubKeyHash);
