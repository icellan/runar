...
pub const P384Wallet = struct {
    ...
    pub fn init(ecdsaPubKeyHash: runar.Addr, p384PubKeyHash: runar.ByteString) P384Wallet {
        return .{ .ecdsaPubKeyHash = ecdsaPubKeyHash, .p384PubKeyHash = p384PubKeyHash };
    }

    pub fn spend(self: *const P384Wallet, p384Sig: runar.ByteString, p384PubKey: runar.ByteString, sig: runar.Sig, pubKey: runar.PubKey) void {
        runar.assert(runar.hash160(pubKey) == self.ecdsaPubKeyHash);
        runar.assert(runar.checkSig(sig, pubKey));
        runar.assert(runar.hash160(p384PubKey) == self.p384PubKeyHash);
