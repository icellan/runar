...
boundary} between six independent compiler implementations. We formalize
...
\begin{enumerate}[label=(\roman*)]
  ...
  \item \textbf{ANF as conformance boundary.} The A-Normal Form intermediate
    ...
    six independent compiler implementations (TypeScript, Go, Rust, Python,
    Zig, Ruby) to produce byte-identical output for the same input.
  ...
  \item \textbf{Deterministic stack scheduling.} A last-use analysis algorithm
    ...
    positions, producing identical bytecode across all six compiler
    implementations.
\end{enumerate}
...
conformance: all six compiler implementations must produce identical
...
between the six independent compiler implementations
(Section~\ref{sec:eval-conformance}), and all six must produce
...
against. Since the six compilers share the same stack lowering and
...
\emph{conformance boundary}: all six compiler implementations must produce
...
conditional multiply steps. The algorithm is identical across all six
...
The six compiler implementations (TypeScript, Go, Rust, Python, Zig, Ruby)
produce byte-identical Bitcoin Script for any valid \runar{} source program.
\end{theorem}

\begin{proof}[Proof sketch]
All six implementations adhere to the same ANF IR specification (which
is canonically serializable) and the same stack lowering algorithm. The
ANF IR serves as the conformance boundary: if all six produce the same
...
\begin{enumerate}[nosep]
  ...
  \item \textbf{Cross-compiler conformance tests} (9 golden-file tests +
    ...
    all six compiler implementations and compares the output against a
...
\caption{Conformance test cases with byte-identical output across
  all six compiler implementations (TypeScript, Go, Rust, Python, Zig, Ruby).}
...
The CI pipeline validates all six compilers independently:
...
\begin{enumerate}[nosep]
  ...
  \item \textbf{Conformance}: A dedicated job (dependent on all six)
...
Deployment SDKs are provided in all six implementation languages
(TypeScript, Go, Rust, Python, Zig, Ruby), enabling contract deployment, method invocation,
...
Both WOTS+ and SLH-DSA script generation are implemented across all six
compilers (TypeScript, Go, Rust, Python, Zig, Ruby) and produce byte-identical output in
...
were originally single-compiler formats, but all six compilers now ship
...
employs six independent implementations with a formal conformance boundary.
...
\begin{enumerate}[label=(\roman*)]
  ...
  \item A \textbf{canonically serializable ANF IR} that serves as a formal
    conformance boundary between six independent compiler implementations,
