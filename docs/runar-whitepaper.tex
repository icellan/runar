...
boundary} between seven independent compiler implementations. We formalize
...
\begin{enumerate}[label=(\roman*)]
  ...
  \item \textbf{ANF as conformance boundary.} The A-Normal Form intermediate
    ...
    seven independent compiler implementations (TypeScript, Go, Rust, Python,
    Zig, Ruby, Java) to produce byte-identical output for the same input.
  ...
  \item \textbf{Deterministic stack scheduling.} A last-use analysis algorithm
    ...
    positions, producing identical bytecode across all seven compiler
    implementations.
\end{enumerate}
...
type P2PKH struct {
  runar.SmartContract
  PubKeyHash runar.Ripemd160Hash `runar:"readonly"`
}
...
natively). Methods live in a plain \texttt{impl} block, and \texttt{pub fn}
versus \texttt{fn} controls method visibility.
...
conformance: all seven compiler implementations must produce identical
...
between the seven independent compiler implementations
(Section~\ref{sec:eval-conformance}), and all seven must produce
...
against. Since the seven compilers share the same stack lowering and
...
\emph{conformance boundary}: all seven compiler implementations must produce
...
conditional multiply steps. The algorithm is identical across all seven
...
The seven compiler implementations (TypeScript, Go, Rust, Python, Zig, Ruby, Java)
produce byte-identical Bitcoin Script for any valid \runar{} source program.
\end{theorem}

\begin{proof}[Proof sketch]
All seven implementations adhere to the same ANF IR specification (which
is canonically serializable) and the same stack lowering algorithm. The
ANF IR serves as the conformance boundary: if all seven produce the same
...
\begin{enumerate}[nosep]
  ...
  \item \textbf{Cross-compiler conformance tests} (9 golden-file tests +
    ...
    all seven compiler implementations and compares the output against a
...
\caption{Conformance test cases with byte-identical output across
  all seven compiler implementations (TypeScript, Go, Rust, Python, Zig, Ruby, Java).}
...
\begin{itemize}[nosep]
  ...
  \item \textbf{Loop unrolling dominates size.} \texttt{MathDemo}
    ...
    (\texttt{sqrt}: 256 Newton rounds, \texttt{pow}: 32 conditional
    multiplies, \texttt{gcd}: 256 Euclidean iterations), resulting in
    14.8~KB of script. This illustrates the size--generality tradeoff
...
The CI pipeline validates all seven compilers independently:
...
\begin{enumerate}[nosep]
  ...
  \item \textbf{Conformance}: A dedicated job (dependent on all seven)
    compiles each conformance test through every compiler, comparing hex
...
Deployment SDKs are provided in all seven implementation languages
(TypeScript, Go, Rust, Python, Zig, Ruby, Java), enabling contract deployment, method invocation,
...
Both WOTS+ and SLH-DSA script generation are implemented across all seven
compilers (TypeScript, Go, Rust, Python, Zig, Ruby, Java) and produce byte-identical output in
...
\paragraph{Go and Rust frontend parity.}
The Go frontend (\texttt{.runar.go}) and the Rust frontend (\texttt{.runar.rs})
were originally single-compiler formats, but all seven compilers now ship
parsers for both, so cross-compiler conformance holds for these formats as
well.
...
employs seven independent implementations with a formal conformance boundary.
...
\begin{enumerate}[label=(\roman*)]
  ...
  \item A \textbf{canonically serializable ANF IR} that serves as a formal
    conformance boundary between seven independent compiler implementations,
