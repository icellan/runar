...
| Java integration suite (run-all.sh) | `integration/run-all.sh:120` | Environmental | When Gradle ≥ 8 is not on `PATH`, `pnpm run integration:all` prints `--- Java: SKIPPED ---` and exits green so local devs without Gradle can still run the rest of the suite. CI bypasses run-all.sh and invokes `gradle test -Drunar.integration=true` directly, so the skip path never fires there. Set `RUNAR_INTEGRATION_STRICT=1` to upgrade the local skip into a hard failure. |
