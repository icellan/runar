...
| `FlipHighBitOfSig` (Rabin adversarial malleability) | `compilers/go/codegen/rabin_adversarial_test.go:381` | Environmental | `t.Skip("sig too small to flip high bit")` — defensive guard inside the bit-flip malleability subtest; fires only when the freshly-signed Rabin signature has `BitLen() < 3` (no high bit to flip while staying `< n`). With the standard ~256-bit test modulus the signature is full-width, so the guard never fires in practice. |
| `analyzer conformance` (per-fixture, Python) | `packages/runar-py/tests/analyzer/test_conformance.py:42,44` | Environmental | `pytest.skip` guards — fire when a conformance fixture lacks an `expected-script.hex` (:42) or the analyzer golden `expected-analyzer-report.json` (:44). Fixtures without analyzer goldens are skipped rather than failing; present fixtures run the full analyzer parity check. |
| `analyzer conformance` (per-fixture, Ruby) | `packages/runar-rb/spec/analyzer/conformance_spec.rb:25,26` | Environmental | RSpec `skip` guards mirroring the Python analyzer suite — fire when a fixture lacks `expected-script.hex` (:25) or the analyzer golden `expected-analyzer-report.json` (:26). |
| `SLH-DSA adversarial bound-violation tests (all 6 parameter sets)` | `packages/runar-testing/src/__tests__/post-quantum-bounds.test.ts:193` | Environmental | `describe.skipIf(!runSlowTests)` — exercises crafted SLH-DSA bound-violation vectors across all 6 FIPS-205 SHA2 parameter sets (expensive keygen/verify). `runSlowTests = IS_CI \|\| RUN_SLOW_TESTS === '1'`; runs in CI, locally set `RUN_SLOW_TESTS=1`. |
