...
//
// The webapp used to keep `var game = &GameState{Phase: "init"}`: one game for
// the whole process, guarded by a mutex. The mutex made concurrent access safe
// and did nothing about the real problem — every visitor was playing the SAME
// game. Two browsers pointed at one server share a round, a threshold, a pair
// of wallets and a contract UTXO; whoever bets second overwrites the first, and
// `/api/state` hands each of them the other's position.
//
