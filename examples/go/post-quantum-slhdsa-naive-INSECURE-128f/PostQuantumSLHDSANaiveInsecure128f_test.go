...
func TestPostQuantumSLHDSANaiveInsecure128f_Compile(t *testing.T) {
	if err := runar.CompileCheck("PostQuantumSLHDSANaiveInsecure128f.runar.go"); err != nil {
...
// VACUOUS. runar.VerifySLHDSA_SHA2_128f is real FIPS 205 verification, not a
...
func TestPostQuantumSLHDSANaiveInsecure128f_RefusesGarbageSignature(t *testing.T) {
	defer func() {
		if recover() == nil {
	...
	c := &PostQuantumSLHDSANaiveInsecure128f{Pubkey: runar.ByteString(make([]byte, 32))}
