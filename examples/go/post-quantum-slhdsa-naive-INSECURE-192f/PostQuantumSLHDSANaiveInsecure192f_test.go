...
func TestPostQuantumSLHDSANaiveInsecure192f_Compile(t *testing.T) {
	if err := runar.CompileCheck("PostQuantumSLHDSANaiveInsecure192f.runar.go"); err != nil {
...
// VACUOUS. runar.VerifySLHDSA_SHA2_192f is real FIPS 205 verification, not a
...
func TestPostQuantumSLHDSANaiveInsecure192f_RefusesGarbageSignature(t *testing.T) {
	defer func() {
		if recover() == nil {
	...
	c := &PostQuantumSLHDSANaiveInsecure192f{Pubkey: runar.ByteString(make([]byte, 32))}
