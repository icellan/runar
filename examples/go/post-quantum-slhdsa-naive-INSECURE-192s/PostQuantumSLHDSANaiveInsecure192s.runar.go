...
type PostQuantumSLHDSANaiveInsecure192s struct {
	runar.SmartContract
	Pubkey runar.ByteString `runar:"readonly"`
}

func (c *PostQuantumSLHDSANaiveInsecure192s) Spend(msg runar.ByteString, sig runar.ByteString) {
	runar.Assert(runar.VerifySLHDSA_SHA2_192s(msg, sig, c.Pubkey))
}
