...
func TestPostQuantumSLHDSANaiveInsecure256f_Compile(t *testing.T) {
	if err := runar.CompileCheck("PostQuantumSLHDSANaiveInsecure256f.runar.go"); err != nil {
...
// VACUOUS. runar.VerifySLHDSA_SHA2_256f is real FIPS 205 verification, not a
...
func TestPostQuantumSLHDSANaiveInsecure256f_RefusesGarbageSignature(t *testing.T) {
	defer func() {
		if recover() == nil {
	...
	c := &PostQuantumSLHDSANaiveInsecure256f{Pubkey: runar.ByteString(make([]byte, 32))}
