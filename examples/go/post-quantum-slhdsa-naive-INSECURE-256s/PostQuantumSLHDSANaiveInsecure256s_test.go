...
func TestPostQuantumSLHDSANaiveInsecure256s_Compile(t *testing.T) {
	if err := runar.CompileCheck("PostQuantumSLHDSANaiveInsecure256s.runar.go"); err != nil {
...
// VACUOUS. runar.VerifySLHDSA_SHA2_256s is real FIPS 205 verification, not a
...
func TestPostQuantumSLHDSANaiveInsecure256s_RefusesGarbageSignature(t *testing.T) {
	defer func() {
		if recover() == nil {
	...
	c := &PostQuantumSLHDSANaiveInsecure256s{Pubkey: runar.ByteString(make([]byte, 32))}
