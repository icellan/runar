...
// are byte-identical and have length 7778. (BUG-100: each of the three public
...
// off and the script goes 9616 -> 7624. R-010 (c2846d87): each covenant method
// now AUTHENTICATES its spender-supplied `_codePart` against the executing
// script rather than trusting it, growing the script from 7624 to 7778.
// Attribution is measured, not inferred: compiling this contract with
// packages/runar-compiler/src/passes/05-stack-lower.ts taken from c2846d87^
// gives 7624 and from c2846d87 gives 7778, with nothing else changed. The two
// later commits that touch that file add nothing here — R-054 (96065132) only
// rejects degenerate checkMultiSig thresholds at compile time, and N-043
// (c21b444f) emits its length pin solely on the variable-length-state path,
// which this fixed-state contract never takes.)
//
