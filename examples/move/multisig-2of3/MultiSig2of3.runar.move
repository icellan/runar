...
// committed array.
//
// `checkMultiSig([sig1, sig2], [contract.pk1, contract.pk2, contract.pk3])`
// lowers to two `array_literal` ANF nodes — one for the signature array, one
// for the pubkey array. This is the canonical site where `array_literal` is
// emitted and is useful as a cross-compiler conformance fixture.
//
// Script layout:
//   Unlocking: <sig1> <sig2>
...
module MultiSig2of3 {
    ...
    use runar::crypto::{checkMultiSig};
    ...
    public fun unlock(contract: &MultiSig2of3, sig1: Sig, sig2: Sig) {
        assert!(checkMultiSig([sig1, sig2], [contract.pk1, contract.pk2, contract.pk3]), 0);
    }
}
