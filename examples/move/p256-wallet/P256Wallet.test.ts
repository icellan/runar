...
const source = readFileSync(join(__dirname, 'P256Wallet.runar.move'), 'utf8');
const FILE_NAME = 'P256Wallet.runar.move';
...
describe('P256Wallet (Move) — compile check', () => {
  it('compiles successfully to Bitcoin Script', () => {
    const result = compile(source, { fileName: FILE_NAME });
    if (!result.success) {
      console.error('P256Wallet (Move) compile errors:', result.diagnostics);
    }
    expect(result.success).toBe(true);
