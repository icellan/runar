...
const source = readFileSync(join(__dirname, 'P384Wallet.runar.move'), 'utf8');
const FILE_NAME = 'P384Wallet.runar.move';
...
function p384Keygen() {
  ...
  const rawPub = spki.subarray(spki.length - 97);
...
describe('P384Wallet (Move) — compile check', () => {
  it('compiles successfully to Bitcoin Script', () => {
    const result = compile(source, { fileName: FILE_NAME });
    if (!result.success) {
      console.error('P384Wallet (Move) compile errors:', result.diagnostics);
    }
    expect(result.success).toBe(true);
