...
// THE FLAW
// --------
// `verifySlhdsaSha2256s` (and any post-quantum signature primitive: WOTS+,
...
// THE CORRECT PATTERN
// -------------------
//   examples/move/post-quantum-wallet/   -- WOTS+ hybrid (ECDSA sig as msg)
//   examples/move/sphincs-wallet/        -- SLH-DSA hybrid (ECDSA sig as msg)
//
// Use those for any real PQ-secured wallet. This file is a teaching artifact.
// =============================================================================

module PostQuantumSLHDSANaiveInsecure256s {
    use runar::types::{ByteString};
    use runar::crypto::{verifySlhdsaSha2256s};

    struct PostQuantumSLHDSANaiveInsecure256s {
        pubkey: ByteString,
    }

    public fun spend(contract: &PostQuantumSLHDSANaiveInsecure256s, msg: ByteString, sig: ByteString) {
        assert!(verifySlhdsaSha2256s(msg, sig, contract.pubkey), 0);
    }
}
