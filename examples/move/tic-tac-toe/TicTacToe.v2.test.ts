...
// surfaces and not in this one. This suite pins the new `.runar.move` file to
...
const v1Sol = here('TicTacToe.runar.move');
const v2Sol = here('TicTacToe.v2.runar.move');
...
describe('TicTacToe v2 (FixedArray) — Move-style surface', () => {
  it('compiles', () => {
    expect(scriptOf(v2Sol, 'TicTacToe.v2.runar.move').length).toBeGreaterThan(0);
  });

  it('is byte-identical to the hand-rolled v1 contract on the same surface', () => {
    expect(scriptOf(v2Sol, 'TicTacToe.v2.runar.move')).toBe(
      scriptOf(v1Sol, 'TicTacToe.runar.move'),
    );
  });
