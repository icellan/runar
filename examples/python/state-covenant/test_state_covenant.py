...
from runar import bb_field_mul, merkle_root_sha256, hash256, cat, sha256
...
def get_proof(layers, index, leaves):
    ...
    for d in range(len(layers) - 1):
        ...
        idx >>= 1
    return b''.join(siblings), leaves[index]


MERKLE_LEAVES = [sha256(bytes([i])) for i in range(16)]
...
def build_advance_args(pre_state_root, new_block_number):
    ...
    batch_data_hash = hash256(cat(pre_state_root, new_state_root))
