...
class MerkleProofDemo < Runar::SmartContract
  prop :expected_root, ByteString

  def initialize(expected_root)
    super(expected_root)
    @expected_root = expected_root
  end

  # Verify a SHA-256 Merkle proof at depth 4.
  runar_public leaf: ByteString, proof: ByteString, index: Bigint
  def verify_sha256(leaf, proof, index)
  ...
  runar_public leaf: ByteString, proof: ByteString, index: Bigint
  def verify_hash256(leaf, proof, index)
