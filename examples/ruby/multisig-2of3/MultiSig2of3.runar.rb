...
class MultiSig2of3 < Runar::SmartContract
  prop :pk1, PubKey
  prop :pk2, PubKey
  prop :pk3, PubKey

  def initialize(pk1, pk2, pk3)
    super(pk1, pk2, pk3)
    @pk1 = pk1
    @pk2 = pk2
    @pk3 = pk3
  ...
  runar_public sig1: Sig, sig2: Sig
  def unlock(sig1, sig2)
    assert check_multi_sig([sig1, sig2], [@pk1, @pk2, @pk3])
  end
end
