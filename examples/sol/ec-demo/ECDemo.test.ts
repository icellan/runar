...
const source = readFileSync(join(__dirname, 'ECDemo.runar.sol'), 'utf8');
const FILE_NAME = 'ECDemo.runar.sol';

/**
 * ECDemo (Solidity) tests — uses real secp256k1 points.
 * See examples/ts/ec-demo/ECDemo.test.ts for detailed explanations.
 */
describe('ECDemo (Solidity)', () => {
  ...
  it('checkX extracts x-coordinate', () => {
    const c = TestContract.fromSource(source, { pt: mockPt }, FILE_NAME);
    expect(c.call('checkX', { expectedX: GxN }).success).toBe(true);
  });

  it('checkX rejects wrong value', () => {
    const c = TestContract.fromSource(source, { pt: mockPt }, FILE_NAME);
    expect(c.call('checkX', { expectedX: 999n }).success).toBe(false);
  });

  it('checkY extracts y-coordinate', () => {
    const c = TestContract.fromSource(source, { pt: mockPt }, FILE_NAME);
    expect(c.call('checkY', { expectedY: GyN }).success).toBe(true);
  });

  it('checkMakePoint constructs point', () => {
    const c = TestContract.fromSource(source, { pt: mockPt }, FILE_NAME);
  ...
  it('checkNegate: y becomes P - Gy', () => {
    const c = TestContract.fromSource(source, { pt: mockPt }, FILE_NAME);
    expect(c.call('checkNegate', { expectedNegY: EC_P - GyN }).success).toBe(true);
  });

  it('checkNegateRoundtrip', () => {
    const c = TestContract.fromSource(source, { pt: mockPt }, FILE_NAME);
    expect(c.call('checkNegateRoundtrip').success).toBe(true);
  });

  it('checkModReduce positive', () => {
    const c = TestContract.fromSource(source, { pt: mockPt }, FILE_NAME);
    expect(c.call('checkModReduce', { value: 17n, modulus: 5n, expected: 2n }).success).toBe(true);
  });

  it('checkModReduce negative', () => {
    const c = TestContract.fromSource(source, { pt: mockPt }, FILE_NAME);
    expect(c.call('checkModReduce', { value: -3n, modulus: 5n, expected: 2n }).success).toBe(true);
  });

  it('checkEncodeCompressed', () => {
    const c = TestContract.fromSource(source, { pt: mockPt }, FILE_NAME);
  ...
  it('checkAddOnCurve: G + 2G on curve', () => {
    const c = TestContract.fromSource(source, { pt: mockPt }, FILE_NAME);
    expect(c.call('checkAddOnCurve', { other: otherPt }).success).toBe(true);
  });

  it('checkMulGenOnCurve: 42*G on curve', () => {
    const c = TestContract.fromSource(source, { pt: mockPt }, FILE_NAME);
