...
const source = readFileSync(join(__dirname, 'P256Wallet.runar.sol'), 'utf8');
const FILE_NAME = 'P256Wallet.runar.sol';
...
function p256Keygen() {
  ...
  const rawPub = spki.subarray(spki.length - 65); // uncompressed point: 04 || x[32] || y[32]
...
describe('P256Wallet (Solidity, Hybrid secp256k1 + P-256)', () => {
...
describe('P256Wallet (Solidity) — compile check', () => {
  it('compiles successfully to Bitcoin Script', () => {
    const result = compile(source, { fileName: FILE_NAME });
    if (!result.success) {
      console.error('P256Wallet (Sol) compile errors:', result.diagnostics);
    }
    expect(result.success).toBe(true);
