...
const source = readFileSync(join(__dirname, 'PostQuantumWOTSNaiveInsecure.runar.sol'), 'utf8');
const FILE_NAME = 'PostQuantumWOTSNaiveInsecure.runar.sol';
...
// `verifyWOTS(msg, sig, pubkey)` only checks (msg, sig) under pubkey; `msg`
// is a free unlocking-script argument unrelated to the spending tx.
describe('PostQuantumWOTSNaiveInsecure (Solidity)', () => {
