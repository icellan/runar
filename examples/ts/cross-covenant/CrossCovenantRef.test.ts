...
const source = readFileSync(join(__dirname, 'CrossCovenantRef.runar.ts'), 'utf8');
...
describe('CrossCovenantRef', () => {
  ...
  describe('verifyAndExtract', () => {
    it('accepts valid output with correct state root', () => {
      const c = TestContract.fromSource(source, { sourceScriptHash: outputHash });
