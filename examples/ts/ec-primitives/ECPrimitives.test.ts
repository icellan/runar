...
const source = readFileSync(join(__dirname, 'ECPrimitives.runar.ts'), 'utf8');

/**
 * ECPrimitives mirrors ECDemo — exercise every secp256k1 builtin against the
 * generator point G and 2*G via the TestContract interpreter (real EC math).
 */
describe('ECPrimitives', () => {
  ...
  it('checkX rejects wrong value', () => {
    const c = TestContract.fromSource(source, { pt: mockPt });
    expect(c.call('checkX', { expectedX: 1n }).success).toBe(false);
  });
