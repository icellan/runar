...
const source = readFileSync(join(__dirname, 'MerkleProofDemo.runar.ts'), 'utf8');
...
describe('MerkleProofDemo', () => {
  ...
  describe('verifySha256 (merkleRootSha256, depth=4)', () => {
    ...
    it('verifies leaf at index 0', () => {
      ...
      const c = TestContract.fromSource(source, { expectedRoot: tree.root });
    ...
    it('rejects wrong index', () => {
      const { proof, leaf } = tree.getProof(0);
      const c = TestContract.fromSource(source, { expectedRoot: tree.root });
      // Use wrong index — should compute a different root
