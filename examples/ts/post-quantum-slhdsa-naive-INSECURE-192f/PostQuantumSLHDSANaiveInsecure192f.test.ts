...
import { TestContract, slhKeygen, slhSign, SLH_SHA2_192f, runSlowTests } from 'runar-testing';
import { compile } from 'runar-compiler';

/**
 * R-193 / R-106 — the 192f SLH-DSA parameter set had no example-layer test.
 *
 ...
 * INSECURE. `verifySLHDSA_SHA2_192f(msg, sig, pubkey)` proves only that `sig`
...
const FILE = 'PostQuantumSLHDSANaiveInsecure192f.runar.ts';
...
describe('PostQuantumSLHDSANaiveInsecure192f (always-on shape checks)', () => {
  it('compiles', () => {
    const r = compile(source, { fileName: FILE });
    expect(r.success, r.diagnostics.map((d) => d.message).join('\n')).toBe(true);
  });

  it('calls the 192f verifier and not another parameter set', () => {
    expect(
      source.includes('verifySLHDSA_SHA2_192f('),
      'the directory name promises 192f; a contract calling a different ' +
        'parameter set here would leave 192f untested while looking covered',
    ).toBe(true);
    const others = ['128s', '128f', '192s', '192f', '256s', '256f'].filter((p) => p !== '192f');
...
describe.skipIf(!runSlowTests)('PostQuantumSLHDSANaiveInsecure192f (real signatures)', () => {
  const params = SLH_SHA2_192f;
