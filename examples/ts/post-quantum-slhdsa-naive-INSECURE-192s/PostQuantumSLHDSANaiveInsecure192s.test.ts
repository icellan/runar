...
import { TestContract, slhKeygen, slhSign, SLH_SHA2_192s, runSlowTests } from 'runar-testing';
import { compile } from 'runar-compiler';

/**
 * R-193 / R-106 — the 192s SLH-DSA parameter set had no example-layer test.
 *
 ...
 * INSECURE. `verifySLHDSA_SHA2_192s(msg, sig, pubkey)` proves only that `sig`
...
const FILE = 'PostQuantumSLHDSANaiveInsecure192s.runar.ts';
...
describe('PostQuantumSLHDSANaiveInsecure192s (always-on shape checks)', () => {
  it('compiles', () => {
    const r = compile(source, { fileName: FILE });
    expect(r.success, r.diagnostics.map((d) => d.message).join('\n')).toBe(true);
  });

  it('calls the 192s verifier and not another parameter set', () => {
    expect(
      source.includes('verifySLHDSA_SHA2_192s('),
      'the directory name promises 192s; a contract calling a different ' +
        'parameter set here would leave 192s untested while looking covered',
    ).toBe(true);
    const others = ['128s', '128f', '192s', '192f', '256s', '256f'].filter((p) => p !== '192s');
...
describe.skipIf(!runSlowTests)('PostQuantumSLHDSANaiveInsecure192s (real signatures)', () => {
