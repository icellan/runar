...
import { TestContract, slhKeygen, slhSign, SLH_SHA2_256f, runSlowTests } from 'runar-testing';
import { compile } from 'runar-compiler';

/**
 * R-193 / R-106 — the 256f SLH-DSA parameter set had no example-layer test.
 *
 ...
 * INSECURE. `verifySLHDSA_SHA2_256f(msg, sig, pubkey)` proves only that `sig`
...
const FILE = 'PostQuantumSLHDSANaiveInsecure256f.runar.ts';
...
describe('PostQuantumSLHDSANaiveInsecure256f (always-on shape checks)', () => {
  it('compiles', () => {
    const r = compile(source, { fileName: FILE });
    expect(r.success, r.diagnostics.map((d) => d.message).join('\n')).toBe(true);
  });

  it('calls the 256f verifier and not another parameter set', () => {
    expect(
      source.includes('verifySLHDSA_SHA2_256f('),
      'the directory name promises 256f; a contract calling a different ' +
        'parameter set here would leave 256f untested while looking covered',
    ).toBe(true);
    const others = ['128s', '128f', '192s', '192f', '256s', '256f'].filter((p) => p !== '256f');
...
describe.skipIf(!runSlowTests)('PostQuantumSLHDSANaiveInsecure256f (real signatures)', () => {
  const params = SLH_SHA2_256f;
