...
import { TestContract, slhKeygen, slhSign, SLH_SHA2_256s, runSlowTests } from 'runar-testing';
import { compile } from 'runar-compiler';

/**
 * R-193 / R-106 — the 256s SLH-DSA parameter set had no example-layer test.
 *
 ...
 * INSECURE. `verifySLHDSA_SHA2_256s(msg, sig, pubkey)` proves only that `sig`
...
const FILE = 'PostQuantumSLHDSANaiveInsecure256s.runar.ts';
...
describe('PostQuantumSLHDSANaiveInsecure256s (always-on shape checks)', () => {
  it('compiles', () => {
    const r = compile(source, { fileName: FILE });
    expect(r.success, r.diagnostics.map((d) => d.message).join('\n')).toBe(true);
  });

  it('calls the 256s verifier and not another parameter set', () => {
    expect(
      source.includes('verifySLHDSA_SHA2_256s('),
      'the directory name promises 256s; a contract calling a different ' +
        'parameter set here would leave 256s untested while looking covered',
    ).toBe(true);
    const others = ['128s', '128f', '192s', '192f', '256s', '256f'].filter((p) => p !== '256s');
...
describe.skipIf(!runSlowTests)('PostQuantumSLHDSANaiveInsecure256s (real signatures)', () => {
