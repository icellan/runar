...
// THE FLAW
// --------
// `verifySLHDSA_SHA2_256s` (and any post-quantum signature primitive:
...
pub const PostQuantumSLHDSANaiveInsecure256s = struct {
    pub const Contract = runar.SmartContract;

    pubkey: runar.ByteString,

    pub fn init(pubkey: runar.ByteString) PostQuantumSLHDSANaiveInsecure256s {
        return .{ .pubkey = pubkey };
    }

    pub fn spend(self: *const PostQuantumSLHDSANaiveInsecure256s, msg: runar.ByteString, sig: runar.ByteString) void {
        runar.assert(runar.verifySLHDSA_SHA2_256s(msg, sig, self.pubkey));
    }
};
