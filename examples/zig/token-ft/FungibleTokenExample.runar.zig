...
pub const FungibleToken = struct {
    ...
    pub fn merge(
        ...
        const outCount = runar.bin2num(runar.cat(runar.substr(otherParentTx, off, 1), pad00));
        runar.assert(outCount >= 1);
        const marker = runar.bin2num(runar.cat(runar.substr(otherParentTx, off + 9, 1), pad00));
        runar.assert(marker == 253);
        const scriptLen = runar.bin2num(runar.cat(runar.substr(otherParentTx, off + 10, 2), pad00));
        const scriptStart = off + 12;
