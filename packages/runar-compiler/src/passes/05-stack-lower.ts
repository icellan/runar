...
class LoweringContext {
  ...
  /**
   * snark_verify — verifies a Groth16 proof on-chain.
   *
   * Currently emits OP_TRUE stub. When a verificationKey is provided via
   * compile options, the real Groth16 verifier can be wired in:
   *
   * 1. The 256-byte _proof ByteString is split into 8 × 32-byte LE field
   *    elements using OP_SPLIT (7 splits). The elements are already in the
   *    correct stack order for the runtime verifier:
   *    [B.x.c0, B.x.c1, B.y.c0, B.y.c1, A.x, A.y, C.x, C.y]
   *
   * 2. Public inputs (e.g., _genesisOutpoint) are hashed to a BN254 field
   *    element and pushed on top.
   *
   * 3. All other stack items are saved to altstack.
   *
   * 4. The runtime verifier ops (generated by generateRuntimeGroth16Verifier
   *    from runar-zkp) are emitted. These expect a clean stack with just the
   *    proof + inputs, and leave 1 (valid) or 0 (invalid) on the stack.
   *
   * 5. Saved items are restored from altstack.
   *
   * The runtime verifier is ~5.2M opcodes (~32MB). The VK is embedded as
   * constants. Use generateRuntimeVerifier() from runar-zkp at the SDK level
  ...
  private lowerSnarkVerify(
    ...
    // Stub: always return true (real verifier wired via SDK)
