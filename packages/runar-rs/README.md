...
Rust wrapper that hides `RunarContract`'s untyped surface. The generated
module exposes:

- A `<Name>Contract` struct wrapping a `RunarContract`.
- A `<Name>ConstructorArgs` struct with named, typed fields.
- One method per public contract method, with typed parameters and a
  `Result<(String, TransactionData), String>` return.
- A `<Name>StatefulCallOptions` for stateful methods.
- A `TerminalOutput` helper for terminal methods (accepts address or raw
  script hex).

Generate at build time and check the result into your repo:

```rust
use std::fs;

fn regenerate_typed_wrapper() -> Result<(), String> {
    let artifact_json = fs::read_to_string("artifacts/Counter.json")
        .map_err(|e| format!("read artifact: {}", e))?;
    let artifact: runar::sdk::RunarArtifact = serde_json::from_str(&artifact_json)
        .map_err(|e| format!("parse artifact: {}", e))?;

    let rust_source = runar::sdk::generate_rust(&artifact);
    fs::write("src/generated/counter.rs", rust_source)
        .map_err(|e| format!("write generated: {}", e))?;
    Ok(())
}
```

Generated wrappers reduce caller boilerplate significantly — you write
`counter.increment(&mut provider, &signer)` instead of
`contract.call("increment", &[], &mut provider, &signer, None)`. The
generated code has no extra runtime cost; it's a thin transparent wrapper.
