// ---------------------------------------------------------------------------
// tsop-cli/commands/compile.ts — Compile TSOP contracts
// ---------------------------------------------------------------------------
 ...
 * 2. Invoke the compiler pipeline (tsop-compiler).
...
export async function compileCommand(
  ...
  try {
    ...
    const sourceEntry = path.resolve(process.cwd(), 'packages/tsop-compiler/src/index.ts');
    ...
    if (!compile) {
      const moduleName = 'tsop-compiler';
  ...
  for (const filePath of files) {
    ...
    if (!compile) {
      console.error(
        '  Error: tsop-compiler is not available. Ensure the package is built and installed.',
      );
      errorCount++;
      continue;
    }
