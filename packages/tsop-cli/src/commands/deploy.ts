// ---------------------------------------------------------------------------
// tsop-cli/commands/deploy.ts — Deploy a compiled contract
// ---------------------------------------------------------------------------
...
import type { TSOPArtifact } from 'tsop-sdk';
...
/**
 * Deploy a compiled TSOP contract to the BSV blockchain.
 *
...
export async function deployCommand(
  ...
  let artifact: TSOPArtifact;
  try {
    const raw = fs.readFileSync(resolvedPath, 'utf-8');
    artifact = JSON.parse(raw) as TSOPArtifact;
  } catch (err) {
  ...
  const contract = new TSOPContract(artifact, constructorArgs);
