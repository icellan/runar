...
describe.skipIf(!hasGo)('Cross-compiler: TS IR -> Go Script', () => {
  ...
  beforeAll(() => {
    tempDir = mkdtempSync(join(tmpdir(), 'tsop-cross-'));
  });
  ...
  try {
    ...
    for (const dir of dirs) {
      ...
      for (const file of files) {
        if (file.endsWith('.tsop.ts')) {
          const source = readFileSync(join(dirPath, file), 'utf-8');
          contracts.push({ name: file.replace('.tsop.ts', ''), source });
        }
      }
    }
  } catch {
...
describe.skipIf(!hasGo)('Cross-compiler: all examples TS IR -> Go', () => {
  ...
  beforeAll(() => {
    tempDir = mkdtempSync(join(tmpdir(), 'tsop-cross-examples-'));
  });
...
function findConformanceTests(): { name: string; sourceFile: string; hexFile: string }[] {
  ...
  try {
    ...
    for (const dir of dirs) {
      ...
      // Find the .tsop.ts source file
      const files = readdirSync(dirPath);
      const tsopFile = files.find(f => f.endsWith('.tsop.ts'));
      if (!tsopFile) continue;

      tests.push({
        name: dir.name,
        sourceFile: join(dirPath, tsopFile),
        hexFile,
      });
    }
  } catch {
...
describe.skipIf(!hasGo)('Cross-compiler conformance: Go output vs golden hex', () => {
  ...
  beforeAll(() => {
    tempDir = mkdtempSync(join(tmpdir(), 'tsop-cross-conformance-'));
  });
