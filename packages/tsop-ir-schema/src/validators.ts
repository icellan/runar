/**
 * Ajv-based validators for TSOP IR schemas.
 *
...
import type { TSOPArtifact } from './artifact.js';
...
function findSchemasDir(): string {
  ...
  throw new Error(
    'Could not locate TSOP IR schema files. Checked: ' + candidates.join(', '),
  );
}
...
export function validateANF(data: unknown): ValidationResult {
  ...
  if (!validate) {
    throw new Error('ANF IR schema not registered — this is a bug in tsop-ir-schema');
  }
  if (validate(data)) {
    return { valid: true };
  }
...
/**
 * Validate a plain object against the TSOP Artifact JSON Schema.
 */
export function validateArtifact(data: unknown): ValidationResult {
  const ajv = getAjv();
  const validate = ajv.getSchema<TSOPArtifact>('artifact.schema.json');
  if (!validate) {
    throw new Error('Artifact schema not registered — this is a bug in tsop-ir-schema');
  }
  if (validate(data)) {
    return { valid: true };
  }
 ...
 * Convenience: validate and throw on failure.
 */
export function assertValidArtifact(data: unknown): asserts data is TSOPArtifact {
  const result = validateArtifact(data);
  if (!result.valid) {
    const summary = result.errors
      .map((e) => `  ${e.path}: ${e.message}`)
      .join('\n');
    throw new Error(`Invalid TSOP Artifact:\n${summary}`);
  }
}
