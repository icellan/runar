...
import type { ContractNode } from 'tsop-ir-schema';
import { TSOPInterpreter } from './interpreter/index.js';
import type { TSOPValue, InterpreterResult } from './interpreter/index.js';
...
function toTSOPValue(val: unknown): TSOPValue {
  ...
  throw new Error(`Cannot convert ${typeof val} to TSOPValue`);
}

function fromTSOPValue(val: TSOPValue): unknown {
  switch (val.kind) {
...
export class TestContract {
  ...
  private readonly interpreter: TSOPInterpreter;

  private constructor(contract: ContractNode, interpreter: TSOPInterpreter) {
  ...
  static fromSource(source: string, initialState: Record<string, unknown> = {}): TestContract {
    ...
    const props: Record<string, TSOPValue> = {};
    for (const [key, value] of Object.entries(initialState)) {
      props[key] = toTSOPValue(value);
    }

    const interpreter = new TSOPInterpreter(props);
  ...
  call(methodName: string, args: Record<string, unknown> = {}): TestCallResult {
    ...
    const tsopArgs: Record<string, TSOPValue> = {};
    for (const [key, value] of Object.entries(args)) {
      tsopArgs[key] = toTSOPValue(value);
    }
    ...
    const outputs: OutputSnapshot[] = rawOutputs.map(out => {
      ...
      for (const [key, val] of Object.entries(out.stateValues)) {
        snapshot[key] = fromTSOPValue(val);
      }
      return snapshot;
    });
  ...
  get state(): Record<string, unknown> {
    const tsopState = this.interpreter.getState();
    const result: Record<string, unknown> = {};
    for (const [key, val] of Object.entries(tsopState)) {
      result[key] = fromTSOPValue(val);
    }
    return result;
  }
