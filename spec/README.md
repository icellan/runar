...
**Formal specification documents for the TSOP language, type system, semantics, and IR format.**

This directory contains the normative documents that define what TSOP is. All compiler implementations -- the TypeScript reference compiler, the Go compiler, and the Rust compiler -- must conform to these specifications.
...
|---|---|---|
| [grammar.md](./grammar.md) | Language grammar in EBNF. Defines the exact TypeScript subset that TSOP accepts: source file structure, contract declarations, property and method syntax, statements, expressions, literals, and the complete list of disallowed TypeScript features. | Draft v0.1.0 |
...
| [semantics.md](./semantics.md) | Operational semantics. Defines how TSOP programs evaluate: the environment and store model, small-step expression evaluation, statement evaluation, assert semantics, method dispatch (single and multiple public methods), private method inlining, state transition semantics (OP_PUSH_TX), the Script execution model, error conditions, and formal properties (termination, determinism, type safety). | Draft v0.1.0 |
